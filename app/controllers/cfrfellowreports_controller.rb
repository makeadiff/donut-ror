class CfrfellowreportsController < ApplicationController
  ...
  def report
    filename = "cfrfellow_report_from_"+session[:from_date_range_cfr_fellow].to_s+"_to_"+session[:to_date_range_cfr_fellow].to_s+".xls"
    ...
    @report_donations = Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
           ...
           session[:from_date_range_cfr_fellow].to_s+" "+$start_time, session[:to_date_range_cfr_fellow].to_s+" "+$end_time, @city[:id])
  ...
  def show
    ...
    if @to_date_str == "\"}" || @start_date_str == "\"}"
      ...
      if params[:from_date].present? && params[:to_date].present?
        session[:from_date_range_cfr_fellow] = @from_date_value
        session[:to_date_range_cfr_fellow] = @to_date_value
      end
      @donations = sort_column
  ...
  def sort_column
    ...
    if session[:from_date_range_cfr_fellow].present? && session[:to_date_range_cfr_fellow].present?
      Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
             joins('inner join cities as cities on cities.id = users.city_id').
            where("donations.created_at >= ? AND donations.created_at <= ? AND users.city_id = ?",  
           session[:from_date_range_cfr_fellow].to_s+" "+$start_time, session[:to_date_range_cfr_fellow].to_s+" "+$end_time, @city[:id]).order(@sort_field + ' ' + sort_direction).
  ...
  def validate_user_role
    validate_user *CfrfellowreportsController.permitted_roles, session[:roles], MadConstants.home_page
  end
  
  private 
  def reset_date_range
    session[:from_date_range_cfr_fellow] = ''
    session[:to_date_range_cfr_fellow] = ''
  end
