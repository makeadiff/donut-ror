$start_time = "00:00:00"
$end_time = "23:59:59"
class NationaltallyeventreportsController < ApplicationController
  ...
  # For this we have defined a excel template @ /views/nationaltallyeventreports/index.xls.erb
  ...
  def report
    ...
    filename = "nationaltallyevent_report_from_"+session[:from_date_range_national_tally].to_s+"_to_"+session[:to_date_range_national_tally].to_s+".xls"
    @report_donations = EventDonation.joins(:donour).joins('inner join users as users on users.id = event_donations.fundraiser_id').
             joins('inner join cities as cities on cities.id = users.city_id').
            where("(event_donations.donation_status = 'RECEIPT SENT' OR event_donations.donation_status = 'DEPOSIT COMPLETE') AND event_donations.updated_at >= ? AND event_donations.updated_at <= ?",  
  ...
  def validate_user_role
    validate_user *NationaltallyeventreportsController.permitted_roles, session[:roles], MadConstants.home_page
  end
