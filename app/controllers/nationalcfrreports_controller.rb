$start_time = "00:00:00"
$end_time = "23:59:59"
class NationalcfrreportsController < ApplicationController
  ...
  def report
    filename = "nationalcfr_report_from_"+session[:from_date_range_national_cfr].to_s+"_to_"+session[:to_date_range_national_cfr].to_s+".xls"
    @report_donations = Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
           ...
           session[:from_date_range_national_cfr].to_s+" "+$start_time, session[:to_date_range_national_cfr].to_s+" "+$end_time)
  ...
  def show
    ...
    if @to_date_str == "\"}" || @start_date_str == "\"}"
      ...
      if params[:from_date].present? && params[:to_date].present?
        session[:from_date_range_national_cfr] = @from_date_value
        session[:to_date_range_national_cfr] = @to_date_value
        ...
        from_date = session[:from_date_range_national_cfr]
        to_date = session[:to_date_range_national_cfr]
  ...
  def sort_column
    ...
    if session[:from_date_range_national_cfr].present? && session[:to_date_range_national_cfr].present?
      Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
             joins('inner join cities as cities on cities.id = users.city_id').
            where("donations.created_at >= ? AND donations.created_at <= ?",
           session[:from_date_range_national_cfr].to_s+" "+$start_time, session[:to_date_range_national_cfr].to_s+" "+$end_time).order(@sort_field + ' ' + sort_direction).
  ...
  def validate_user_role
    validate_user Role.NATIONAL_CFR_HEAD, Role.ADMINISTRATOR,session[:roles],MadConstants.home_page
  end
  
  private 
  def reset_date_range
    session[:from_date_range_national_cfr] = ''
    session[:to_date_range_national_cfr] = ''
  end
end
