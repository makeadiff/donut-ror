class EventsfellowreportsController < ApplicationController
  ...
  def report
    filename = "eventsfellow_report_from_"+session[:from_date_range_events_fellow].to_s+"_to_"+session[:to_date_range_events_fellow].to_s+".xls"
    @events_city = City.find_by_id(User.select("city_id").where(:id => session[:session_user]))
    @report_eventdonations = EventDonation.joins(:donour).joins(:event).joins('inner join users as users on users.id = event_donations.fundraiser_id').
             where("event_donations.created_at >= ? AND event_donations.created_at <= ? and users.city_id = ?",  
           session[:from_date_range_events_fellow].to_s+" "+$start_time, session[:to_date_range_events_fellow].to_s+" "+$end_time, @events_city[:id])
  ...
  def show
    ...
    if @to_date_str == "\"}" || @start_date_str == "\"}"
      ...
      if params[:from_date].present? && params[:to_date].present?
        session[:from_date_range_events_fellow] = @from_date_value
        session[:to_date_range_events_fellow] = @to_date_value
      end
      @eventdonations = sort_column
    ...
    @from_date_range = session[:from_date_range_events_fellow]
    @to_date_range = session[:to_date_range_events_fellow]
  end
  ...
  def sort_column
    ...
    if session[:from_date_range_events_fellow].present? && session[:to_date_range_events_fellow].present?
      EventDonation.joins(:donour).joins(:event).joins('inner join users as users on users.id = event_donations.fundraiser_id').
             where("event_donations.created_at >= ? AND event_donations.created_at <= ? and users.city_id = ?",  
           session[:from_date_range_events_fellow].to_s+" "+$start_time, session[:to_date_range_events_fellow].to_s+" "+$end_time, @events_city[:id]).order(@sort_field + ' ' + sort_direction).
  ...
  def validate_user_role
    validate_user *EventsfellowreportsController.permitted_roles, session[:roles], MadConstants.home_page
  end
  
  private 
  def reset_date_range
    session[:from_date_range_events_fellow] = ''
    session[:to_date_range_events_fellow] = ''
  end
