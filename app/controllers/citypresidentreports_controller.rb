class CitypresidentreportsController < ApplicationController
  
  before_filter :validate_user_role, :reset_date_range, :except => :report
  @permitted_roles = [Role.CITY_PRESIDENT]
  ...
  def report
    filename = "cityprecident_report_from_"+session[:from_date_range_city_president].to_s+"_to_"+session[:to_date_range_city_president].to_s+".xls"
    ...
    @report_donations = Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
            ...
            where("donations.created_at >= ? AND donations.created_at <= ? AND users.city_id = ?",  
           session[:from_date_range_city_president].to_s+" "+$start_time, session[:to_date_range_city_president].to_s+" "+$end_time, @city[:id])
  ...
  def show
    ...
    if @to_date_str == "\"}" || @start_date_str == "\"}"
      ...
      if params[:from_date].present? && params[:to_date].present?
        session[:from_date_range_city_president] = @from_date_value
        session[:to_date_range_city_president] = @to_date_value
      end
      @donations = sort_column
      ...
      from_date = session[:from_date_range_city_president]
      to_date = session[:to_date_range_city_president]  
    end
  end
  ...
  def sort_column
    ...
    if session[:from_date_range_city_president].present? && session[:to_date_range_city_president].present?
      Donation.joins(:donour).joins('inner join users as users on users.id = donations.fundraiser_id').
             joins('inner join cities as cities on cities.id = users.city_id').
            where("donations.created_at >= ? AND donations.created_at <= ? AND users.city_id = ?",  
           session[:from_date_range_city_president].to_s+" "+$start_time, session[:to_date_range_city_president].to_s+" "+$end_time, @city[:id]).order(@sort_field + ' ' + sort_direction).
  ...
  def validate_user_role
    validate_user *CitypresidentreportsController,permitted_roles, session[:roles], MadConstants.home_page
  end
  
  private 
  def reset_date_range
    session[:from_date_range_city_president] = ''
    session[:to_date_range_city_president] = ''
  end
